...
export function ProductCard({ product, onBuyClick }: ProductCardProps) {
  ...
  // If accountType is explicitly set, use it; otherwise infer from totalSlots
  const totalSlots = product.totalSlots ?? 1;
  const availableSlots = product.availableSlots ?? totalSlots;
  // 'profile' if multiple slots, 'full' if single slot
  const isFullAccount = product.accountType
    ...
    <Card className="overflow-hidden border-none shadow-lg hover:shadow-xl transition-all duration-300 group bg-white dark:bg-slate-900 rounded-3xl">
      ...
      <div className={`h-32 relative flex items-center justify-center ${!product.imageUrl ? style.bg : 'bg-slate-100 dark:bg-slate-800'}`}>
        ...
        <Badge className="absolute top-3 right-3 bg-green-100 text-green-700 hover:bg-green-200 border-none font-bold text-[10px] uppercase tracking-wider z-10">
          {availableSlots > 0 ? 'STOCK ONLINE' : 'AGOTADO'}
        </Badge>
      ...
      <CardContent className="p-5">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {isFullAccount ? (
            <span className="bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-xs font-bold px-2.5 py-1 rounded-md flex items-center gap-1">
              <User size={12} />
              Cuenta Completa
            </span>
          ) : (
            <span className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 text-xs font-bold px-2.5 py-1 rounded-md flex items-center gap-1">
              <Users size={12} />
              {availableSlots}/{totalSlots} Perfiles
            </span>
          )}
        </div>
        ...
        <div className="flex items-end justify-between mt-2">
          ...
          <Button
            ...
            disabled={availableSlots <= 0 && !isFullAccount}
            className="h-10 w-10 rounded-full p-0 bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
