...
/**
 * GET /api/affiliate/purchases
 *
 * List affiliate's purchases with filtering and pagination.
 *
 * Query params:
 ...
 *
 * Response 200: Paginated list of purchases with stats
 */
export async function GET(request: NextRequest) {
    try {
        ...
        const where: any = {
            order: {
                userId: user.id, // Affiliate is the buyer
            },
        };
        ...
        const data = orderItems.map((item: any) => {
            ...
            return {
                ...
                sellerId: item.order.user.id, // The buyer (affiliate in this case)
...
/**
 * POST /api/affiliate/purchases
 *
 * Purchase a product from the marketplace as an affiliate.
 *
 * Headers:
 * - Authorization: Bearer <token>
 *
 ...
 * }
 *
 * Response 201: Purchase details with credentials
 */
...
export async function POST(request: NextRequest) {
    try {
        ...
        // 6. Update Inventory - Decrement available slots
        ...
        if (inventoryAccount) {
            ...
            if (inventoryAccount.slots.length > 0) {
                ...
                console.log(`[Affiliate Purchase] Slot ${slotToSell.id} assigned to OrderItem ${orderItemId}. Remaining slots: ${remainingSlots}`);
                ...
                if (remainingSlots <= 0) {
                    ...
                    console.log(`[Affiliate Purchase] Product ${productId} deactivated - all slots sold`);
                }
            } else {
                ...
                console.log(`[Affiliate Purchase] Full account sold. Product ${productId} deactivated.`);
            }
        }

        // 7. Return purchase result
        return NextResponse.json(result, { status: 201 });
    } catch (error: any) {
        console.error('Error creating affiliate purchase:', error);
