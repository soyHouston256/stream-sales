...
/**
 * POST /api/affiliate/purchases/[id]/dispute
 *
 * Crea una disputa sobre una compra completada (para affiliates)
 *
 * Body:
 * {
...
export async function POST(
  ...
  try {
    ...
    // 2. Verificar rol de affiliate
    if (decoded.role !== 'affiliate') {
      return NextResponse.json(
        { error: 'Forbidden. Only affiliates can create disputes on purchases.' },
        { status: 403 }
      );
    }
    ...
    console.error('[POST /api/affiliate/purchases/[id]/dispute] Error:', error);
    ...
    if (
      ...
      error.message.includes('Only the affiliate') ||
