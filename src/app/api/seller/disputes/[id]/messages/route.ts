...
export async function GET(
  ...
  try {
    ...
    console.error('[GET /api/seller/disputes/[id]/messages] Error:', error);
    return NextResponse.json(
...
/**
 * POST /api/seller/disputes/[id]/messages
 *
...
export async function POST(
  ...
  try {
    ...
    if (decoded.role !== 'seller') {
      return NextResponse.json(
        { error: 'Forbidden. Only sellers can send messages.' },
        { status: 403 }
      );
    }
    ...
    const result = await useCase.execute({
      ...
      senderRole: 'seller',
      message,
      isInternal: false, // Seller solo puede crear mensajes públicos
      attachments,
    });
    ...
    console.error('[POST /api/seller/disputes/[id]/messages] Error:', error);
